...
import { Sparkles, BookOpen, List, Zap, Loader2, CheckCircle } from "lucide-react";
...
export default function GenerateQuestionsPage() {
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      setGenerating(true);
      const data = await adminApi.generateQuestions(selectedTopic, questionCount, {
  ...
  const breadcrumbs = [{ label: "AI Question Creator", href: "/ai-questions" }, { label: "Generate Questions" }];

  return (
    <AppLayout sidebarItems={appMenuItems} pageTitle="Generate Questions" breadcrumbs={breadcrumbs}>
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select a topic and configure your question generation preferences</CardDescription>
          </CardHeader>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select value={selectedLesson} onValueChange={setSelectedLesson}>
                  ...
                  <SelectContent>
                    {lessons.map((lesson) => (
                      <SelectItem key={lesson._id || lesson.id} value={lesson._id || lesson.id || ""}>
            ...
            <div className="flex items-center justify-between pt-4">
              <div className="text-sm text-gray-600">
                {selectedLesson && selectedTopic && (
                  <span>
                    Generating questions for <strong>{getSelectedTopicTitle()}</strong> in{" "}
        ...
        {generatedQuestions.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Review and manage your AI-generated questions</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {generatedQuestions.map((question, index) => (
                  <Card key={question._id || question.id || index} className="border-l-4 border-l-purple-500">
                    <CardHeader>
                      <CardTitle className="text-lg">Question {index + 1}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <p className="font-medium">{question.question || question.questionText}</p>
                        ...
                        {question.explanation && (
                          <div className="mt-3 p-3 bg-background rounded-md border border-blue-200 dark:border-blue-700">
                            <p className="text-sm font-medium text-blue-700 dark:text-blue-200">Explanation:</p>
                            <p className="text-sm text-blue-600 dark:text-blue-300">{question.explanation}</p>
                          </div>
                        )}

                        <div className="flex justify-between items-center text-sm text-gray-500">
                          <span>Type: {question.type || question.questionType}</span>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Choose a lesson and topic for question generation</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-8 h-8 bg-background rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Set the number of questions to generate</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-8 h-8 bg-background rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">AI creates questions that you can review and use</p>
                </div>
              </div>
            </div>
          </CardContent>
